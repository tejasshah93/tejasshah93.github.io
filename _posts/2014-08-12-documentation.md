...
```
which provides an output in the form of ```git diff```. Iteratively fetching all the '+' differences from the ```diff``` output would give the dynamic HTML content generated by all the triggers scattered across different HTML files. All such positive diffs are merged into a file say **dynamicDOMElements.html**.  
<br/>
...
However, while getting the union of dynamicDOMElements with source code, there needs to be some HTML headers attached with dynamic content else it would lead to false validation of that content via AChecker stating some problem types of HTML headers although with dynamically obtained HTML content although it might be alright in the original source code.  
In a nutshell, false validations regarding HTML headers( ```<!DOCTYPE>``` headers and ```<html>``` attributes) w.r.t to validating new content must not be given as output by AChecker since the new content from dynamicDOMElements is unified with original source code disjointly.  
<br/>
*SOLUTION*:  
While creating _dynamicDOMElements_ file itself, using BeautifulSoup Python library, we get the DOCTYPE declaration and HTML attributes like xmlns, xml:lang, lang and prepend them accordingly in the right format before the contents of dynamicDOMElements.  
Thus, in this way it is ensured that no problem reporting will be introduced because of missing HTML headers/attributes while validating new content.

**Result**:
Thus, with performing above steps now we have __dynamicDOMElements.html__ file with merged contents which were generated by triggering DOM manipulating elements. Also, this file contains apt DOCTYPE headers and html attributes (same as that of the original webpage), thus leading to no ambiguous warnings/problems reporting from AChecker.
